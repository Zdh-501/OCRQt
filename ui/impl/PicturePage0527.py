...
class PicturePage(QtWidgets.QWidget, Ui_PicturePage):
    ...
    def __init__(self):
        ...
        # 初始化 PaddleOCR 配置
        self.det_model_dir = "D:/Paddle/inference_model/det/det_db_inference"
        self.rec_model_dir = "D:/Paddle/inference_model/rec/rec0508"
    ...
    def updateTextBrowser(self, item_details):
        ...
        print("相机参数",self.camera_parameters)
    ...
    def onOcrFinished(self, results):
        ...
        for result_group in results:
            if result_group and result_group[1]:
                            ...
                            for item in items:
                                if isinstance(item, tuple) and len(item) == 2:
                                    ...
                                    if isinstance(text, str):
                                        all_texts.append(text.strip())  # 确保是字符串并去除空格
        ...
        if len(all_texts) == 1:
            ...
            if len(text) > 7 and ' ' in text:
                parts = text.split(' ', 1)
                batch_numbers.append(parts[0])
                dates.append(parts[1])
            else:
                batch_numbers.append(text)  # 单独的批号
        else:
            # 多个元素，第一个作为批号，其他作为日期
            batch_numbers.append(all_texts[0])
            dates.extend(all_texts[1:])

        print("提取的日期:", dates)
        print("提取的批号:", batch_numbers)
        if len(batch_numbers)+len(dates)<3:
            # 获取当前时间
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

            # 错误信息
            error_message = "单次检测任务中，未识别到三条有效信息"
        ...
        # 显示日期
        for date in dates:
            self.textBrowser_4.append(f"日期: {date}")
        # # 根据日期数量显示不同的文本
        # if len(dates) == 1:
        #     date_in_list = None
